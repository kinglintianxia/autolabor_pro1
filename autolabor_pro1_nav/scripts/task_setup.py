...
def setup_task_environment(self):
    ...
    self.square_size = rospy.get_param("~square_size", 0.5) # meters
    
    # Set the low battery threshold (between 0 and 100)
    self.low_battery_threshold = rospy.get_param('~low_battery_threshold', 10)
    
    # How many times should we execute the patrol loop
    self.n_patrols = rospy.get_param("~n_patrols", 1)
    ...
    ## modify this
    self.waypoints.append(Pose(Point(0.0, 0.0, 0.0), quaternions[3]))
    self.waypoints.append(Pose(Point(2.45, -0.25, 0.0), quaternions[0]))
    self.waypoints.append(Pose(Point(2.45, 0.75, 0.0), quaternions[1]))
    self.waypoints.append(Pose(Point(-1.35, 0.1, 0.0), quaternions[2]))
    ...
    self.docking_station_pose = (Pose(Point(1.0, 0.0, 0.0), Quaternion(0.0, 0.0, 0.0, 1.0)))            
...
def init_docking_station_marker(self):
    ...
    self.docking_station_marker.header.frame_id = 'map'
